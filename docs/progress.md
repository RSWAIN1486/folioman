...
- Replaced the fixed dashboard 1D-return card with a return-window selector (1 day through all time) that shows absolute return plus annualized % p.a. in the shared web/desktop frontend.
...
- The investor dashboard's return-window card is computed client-side from the persisted value series plus the investor's ledger cashflows; all-time reuses the backend XIRR headline.
